...
/// Inserts advert blocks depending on current browser language.
function insertAdvertBlocks() {
  // noinspection JSDeprecatedSymbols, get browser language
  const browserLanguage = (navigator.language || navigator.userLanguage).toLowerCase();
...
//          renderTo: document.getElementsByClassName('yandex-advert-block')[0].id,
//          blockId: document.getElementsByClassName('yandex-advert-block')[0].id.split('_')[2],
//        })
//      });
//    } else {
...
function adjustWindowForNewSize () {
  ...
  window.addEventListener('resize', function() {
    try {
       document.querySelector('html').style.zoom = (window.innerWidth / 1920).toPrecision(2);
       document.querySelector('html').style.MozTransform = `scale(${(window.innerWidth / 1920).toPrecision(2)})`;
    } catch (_) {
      /* Do nothing */
    }
  }, true);
...
function openScreenshot(index) {
  try {
    ...
    if(!src.includes('screenshot_')) {
      ...
      document.getElementsByClassName('screenshot-object')[0].src = src;
    }
  } catch (_) {
    /* Do nothing */
  }
...
function navigateToTranslatedWebsite(language, saveLanguageSwitchingState = false) {
  try {
    ...
    var ending = currentURL.split(".net/").pop();
...
function translateWebsite(language, saveLanguageSwitchingState = false) {
  ...
  try {
    ...
    if (language === 'Русский' || language === 'Українська') {
      image.src = "resources/main_ru.png";
    } else {
      image.src = "resources/main_en.png";
    }
  } catch (_) {
    /* Do nothing */
  }
